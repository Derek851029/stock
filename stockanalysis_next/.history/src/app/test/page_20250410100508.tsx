            ...
            <div className="flex flex-col gap-3 mt-5">
                {data.map((item: dataItem) => (
                    <div key={item.id} className="flex gap-2 items-center">
                        <input type="text" className="border border-gray-300 rounded p-2" placeholder="Enter text" value={item.text} onChange={(e) => {
                            const newData = [...data];
                            newData[item.id].text = e.target.value;
                            setData(newData);
                        }} />
                        <button onClick={() => {
                            const newData = data.filter((_, index) => index !== item.id);
                            setData(newData);
