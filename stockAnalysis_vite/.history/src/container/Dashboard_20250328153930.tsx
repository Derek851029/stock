...
const Dashboard = () => {
  const [topGainers, setTopGainers] = useState<TopGainersItem[]>([]);
  ...
  const getTopGainers = async () => {
    const data = await api.get<TopGainersData>("/dashboard/topGainers");
  ...
  const getIndices = async () => {
    const data: any = await api.get("/dashboard/indices");

    if (data) {
      const indicesData: IndicesData = data.data;
                  ...
                  >
                    ...
                    <LineSeriesChart
                      lineData={indices[indicesName as keyof IndicesData].map(
                        (item) => ({ time: item.timestamp, value: item.price })
                      )}
                      volumeData={indices[indicesName as keyof IndicesData].map(
                        (item) => ({ time: item.timestamp, value: item.volume })
                      )}
                    />
